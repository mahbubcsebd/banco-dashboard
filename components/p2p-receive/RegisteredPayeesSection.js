...
import Button from '../login/Button'; // Assuming your custom button
...
const RegisteredPayeesSection = ({
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mx-auto"
    >
      {/* Top Section: Register Button */}
      <Link
        href="/dashboard/register-recieved-money"
      ...
      >
        <Button
          ...
          onClick={onRegister}
          size="default"
          className="text-sm border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-white"
        >
          Register to receive money
        </Button>
      </Link>

      {/* Payee List Header (Desktop Only) */}
      ...
      <div className="space-y-2">
        {data.map((payee, index) => (
          ...
          >
            {/* Nickname (Full width on mobile, 1.5fr on desktop) */}
            <div className="col-span-1 md:col-span-1">
              ...
              <div className="md:hidden text-xs text-gray-600 mt-1 space-y-0.5">
                ...
                <p>
                  ...
                  {payee.receiveAccount}
                </p>
              </div>
            </div>

            {/* Email/Phone (Hidden on mobile, shown on desktop) */}
            <span className="hidden md:block text-gray-700 truncate">
              {payee.emailPhone}
            </span>

            {/* Receive Account (Hidden on mobile, shown on desktop) */}
            <span className="hidden md:block text-gray-700">
              {payee.receiveAccount}
            </span>

            {/* Checkbox/Selection Indicator (Rightmost) */}
            <div className="flex justify-end pr-2">
              {isPayeeSelected(payee.id) ? (
                <Check className="w-5 h-5 text-blue-600 bg-blue-100 rounded-full" />
              ) : (
                <div className="w-5 h-5 border border-gray-400 rounded-full bg-white"></div>
              )}
            </div>
          </motion.div>
        ))}
