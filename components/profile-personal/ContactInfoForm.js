...
const ContactInfoForm = ({ initialData, onSubmit, isSubmitting = false }) => {
  ...
  const validate = () => {
    ...
    // Required fields based on the image (asterisk next to Mobile and Email)
    // Assuming First Name, Last Name, Address 1 are also typically required in banking forms
    ...
    // Fields marked with * in the image (Mobile Number and Email Address)
  ...
  const handleCancel = () => {
    // Optionally revert to initial data or navigate back
    setFormData(initialData);
    setErrors({});
